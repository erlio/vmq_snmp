...
exometer_init(Opts) ->
    lager:info("~p(~p): Starting~n", [?MODULE, Opts]),
    RunningApps = application:which_applications(),
    case lists:keymember(snmp, 1, RunningApps) of
        true ->
            ok;
        false ->
            lager:warning("Application SNMP not started. Ensure that a usable SNMP agent is configured.")
    end,
...
exometer_report(Metric, DataPoint, _Extra, Value, St)  ->
    lager:debug("Report metric ~p_~p = ~p~n", [Metric, DataPoint, Value]),
...
snmp_operation(Op, Val, Key) ->
    lager:warning("Unhandled SNMP operation ~p on ~p with value ~p", [Op, Key, Val]),
    {noValue, noSuchObject}.
        ...
        {ok, BinMib0} ->
            ...
            case IgnoreUnload of
                ...
                false ->
                    ...
                    lager:info("MIB ~s unloaded", [BinMib1])
            end,
            case snmpa:load_mibs(snmp_master_agent, [BinMib1]) of
                ok ->
                    lager:info("MIB ~s loaded", [BinMib1]),
                    {ok, increment_vsn(Vsn)};
                E ->
                    lager:error("Error ~p when loading MIB ~s", [E, BinMib1]),
                    E
            end;
        E ->
            lager:error("Error ~p when compiling MIB ~s", [E, Mib0]),
            E
    end.
